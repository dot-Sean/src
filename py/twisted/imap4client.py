...
# API docs for "twisted.mail.imap4.IMAP4Client" http://twistedmatrix.com/documents/8.2.0/api/twisted.mail.imap4.IMAP4Client.html
...
class SimpleIMAP4ClientFactory(protocol.ClientFactory):
    ...
    def buildProtocol(self, addr):
        ...
        #auth = imap4.PLAINAuthenticator(self.username)
        #auth = imap4.LOGINAuthenticator(self.username)
...
def cbMailboxList(result, proto):
    ...
    s = '\n'.join(['%d. %s' % (n + 1, m) for (n, m) in
     zip(range(len(result)), result) ])
    if not s:
...
def cbExamineMbox(result, proto):
    ...
    return proto.fetchSpecific('1:*',
     headerType='HEADER.FIELDS', headerArgs=['SUBJECT'] ). \
